...
public final class IBeaconManufacturerData {
	...
	private static String calculateUUIDString(final byte[] uuid){
		...
		for(int i = 0 ; i< uuid.length; i++){
			...
			if(i == 8){sb.append('-');}
			if(i == 10){sb.append('-');}
			//original. 0이 포함된 uuid에서는 0이 누락됨.
//			sb.append(Integer.toHexString(ByteUtils.getIntFromByte(uuid[i])));

			//수정
			String str = Integer.toHexString(ByteUtils.getIntFromByte(uuid[i]));
			if (str.length() < 2) {
				str = "0" + str;
			}
			sb.append(str);
