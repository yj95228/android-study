...
public final class AdRecord implements Parcelable{
	...
	private static String getHumanReadableAdType(int type){
		if (type == TYPE_CONNECTION_INTERVAL_RANGE) {
			return "Slave Connection Interval Range";
		} else if (type == TYPE_DEVICE_CLASS) {
			return "Class of device";
		} else if (type == TYPE_FLAGS) {
			return "Flags";
		} else if (type == TYPE_MANUFACTURER_SPECIFIC_DATA) {
			return "Manufacturer Specific Data";
		} else if (type == TYPE_LOCAL_NAME_COMPLETE) {
			return "Name (Complete)";
		} else if (type == TYPE_LOCAL_NAME_SHORT) {
			return "Name (Short)";
		} else if (type == TYPE_SECURITY_MANAGER_OOB_FLAGS) {
			return "Security Manager OOB Flags";
		} else if (type == TYPE_SERVICE_UUIDS_LIST_128BIT) {
			return "Service UUIDs (128bit)";
		} else if (type == TYPE_SERVICE_UUIDS_LIST_16BIT) {
			return "Service UUIDs (16bit)";
		} else if (type == TYPE_SERVICE_DATA) {
			return "Service Data";
		} else if (type == TYPE_SIMPLE_PAIRING_HASH_C) {
			return "Simple Pairing Hash C";
		} else if (type == TYPE_SIMPLE_PAIRING_RANDOMIZER_R) {
			return "Simple Pairing Randomizer R";
		} else if (type == TYPE_TK_VALUE) {
			return "TK Value";
		} else if (type == TYPE_TX_POWER_LEVEL) {
			return "Transmission Power Level";
		} else if (type == TYPE_UUID128) {
			return "Complete list of 128-bit UUIDs available";
		} else if (type == TYPE_UUID128_INC) {
			return "More 128-bit UUIDs available";
		} else if (type == TYPE_UUID16) {
			return "Complete list of 16-bit UUIDs available";
		} else if (type == TYPE_UUID16_INC) {
			return "More 16-bit UUIDs available";
		} else if (type == TYPE_UUID32) {
			return "Complete list of 32-bit UUIDs available";
		} else if (type == TYPE_UUID32_INC) {
